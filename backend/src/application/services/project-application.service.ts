...
export class ProjectApplicationService {
   ...
   * 创建项目
   */
  async createProject(command: CreateProjectCommand): Promise<ApplicationResult<{ projectId: string }>> {
    try {
      // 验证用户存在
   ...
   * 查询用户项目列表
   */
  async getUserProjects(query: GetUserProjectsQuery): Promise<ApplicationResult<{
    projects: Project[];
    total: number;
    page: number;
    limit: number;
   ...
   * 查询项目统计信息
   */
  async getProjectStats(query: GetProjectStatsQuery): Promise<ApplicationResult<{
    total: number;
    published: number;
    draft: number;
    archived: number;
  ...
  async getRecentProjects(query: GetRecentProjectsQuery): Promise<ApplicationResult<Project[]>> {
    try {
      const projects = await this.projectRepository.findRecentlyUpdated(
        query.userId,
        query.limit
      );
